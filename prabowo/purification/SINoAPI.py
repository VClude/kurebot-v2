...
class BaseAPI():
    ...
    def __init__(self, user_id: int, app_version: str, payment_id: str, prv_key: str, sess: str, debug = False):
...
class API(BaseAPI):
    ...
    def POST__api_gvg_sp(self, tot: int):
        payload = {
            "recoverySpValue": tot
        }
        r = self._post("/api/gvg/end_sp_recovery", payload)
        return r
    ...
    def POST__api_gvg_out_battle_get_battle_history(self, battleNum: int, gvgDataId: int, pageNo: int, type: int):
        payload = {
            "battleNum": battleNum,
            "gvgDataId": gvgDataId,
            "pageNo": pageNo,
            "type": type
            # 0 for all, 4 for summon
        }
        r = self._post("/api/gvg_out_battle/get_battle_history", payload)
        return r

    def POST__api_gvg_out_battle_get_battle_user_list(self, gvgDataId: int, battleNum: int):
        payload = {
            "gvgDataId": gvgDataId,
            "battleNum": battleNum,
        }
        r = self._post("/api/gvg_out_battle/get_battle_user_list", payload)
        return r
    ...
    def POST__api_quest_battle_initialize_stage(self):
